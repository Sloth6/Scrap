...
parseVideoUrl = (url) ->
  ...
  [_, collectionKey, key, name] = keys
  { collectionKey, key, name }
...
module.exports = (video_url, callback) ->
  ...
  try
    { collectionKey, key, name } = parseVideoUrl video_url
  catch e
  ...
  cmd = ffmpeg(video_url)
    ...
    .on 'end', () ->
      ...
      fs.readFile temp_path, (err, data) ->
        ...
        upload_options = { key, img:data, collectionKey, path:name, type: 'png' }
